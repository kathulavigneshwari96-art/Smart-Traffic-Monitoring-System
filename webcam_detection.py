...
while True:
            ...
            if class_name in allowed_classes and confidence > 0.5:
                ...
                if class_name in vehicle_classes:
                    ...
                    if track_id not in logged_ids:
                        csv_writer.writerow(
                            [time.strftime("%Y-%m-%d"), time.strftime("%H:%M:%S"), class_name, track_id]
                        )

                    logged_ids.add(track_id)

                    log_file.write(f"{time.ctime()} - {class_name}\n")
                    ...
                    try:
                        ...
                        if vehicle_crop.size > 0:
                            ...
                            for res in ocr_results:
                                ...
                                plate_text = re.sub(r"[^A-Z0-9]", "", detected_text.upper())
    ...
    cv2.putText(frame, f"Vehicles: {vehicle_count}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)

    # Traffic status display
    cv2.putText(frame, traffic_status, (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 3)
    ...
    cv2.putText(frame, f"FPS: {int(fps)}", (20, 120), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 3)

    # Person count display
    cv2.putText(frame, f"Persons: {person_count}", (20, 160), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 255), 3)
    cv2.putText(frame, f"Total Vehicles: {total_vehicle_count}", (20, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 3)

    # Save video
    out.write(frame)

    # Show output
    cv2.imshow("Smart Traffic Vehicle Detection", frame)
